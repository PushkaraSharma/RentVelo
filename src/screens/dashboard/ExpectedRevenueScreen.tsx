...
import { Building2, Users, AlertTriangle, ChevronRight, Zap, Clock, Banknote, Plus } from 'lucide-react-native';
...
export default function ExpectedRevenueScreen({ navigation }: any) {
    ...
    const { composition, totalExpected, properties, highRiskTenants, propertyCount } = data;
    const isEmptyRevenue = totalExpected === 0 && properties.length === 0 && highRiskTenants.length === 0;
    ...
    return (
        <View style={[styles.container, { paddingTop: insets.top }]}>
            ...
            <ScrollView
                ...
                contentContainerStyle={[styles.scrollContent, isEmptyRevenue && styles.emptyScrollContent]}
            >
                {isEmptyRevenue ? (
                    <View style={styles.emptyState}>
                        <View style={styles.emptyIconBox}>
                            <Building2 size={36} color={theme.colors.accent} />
                        </View>
                        <Text style={styles.emptyTitle}>{hasProperties ? 'No Expected Revenue Yet' : 'No Properties Yet'}</Text>
                        <Text style={styles.emptySubtitle}>
                            {hasProperties
                                ? 'Add tenants and generate rent bills to see your expected revenue for this month.'
                                : 'Create your first property to start tracking monthly rent, dues, and collections.'}
                        </Text>
                        <Button
                            title={hasProperties ? 'View Properties' : 'Add First Property'}
                            onPress={() => hasProperties ? navigation.navigate('Main', { screen: 'Properties' }) : navigation.navigate('AddProperty')}
